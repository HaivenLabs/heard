...
function check(repoRoot){
  ...
  const files=walk(repoRoot,[],['node_modules','.git','.cache','dist','.next','build','coverage','.haiven']); for(const f of files){ const r=rel(f,repoRoot), base=path.basename(f); if(['.env','.env.local','.env.development','.env.production','.env.test'].includes(base)) errors.push(`Secret-like environment file found: ${r}`); if(/migrations?\//i.test(r) && !/^(\d{3,}|\d{4}[_-]\d{2}[_-]\d{2}|v\d+)[_-].+\.(sql|ts|js|go)$/i.test(base)) warnings.push(`Migration file may not be versioned: ${r}`); if(/\.(ts|tsx|js|jsx|go|py|cs|java)$/i.test(f)){ const c=read(f); if(/console\.log\([^)]*(password|secret|token|apiKey|api_key)/i.test(c)) errors.push(`Possible sensitive logging found: ${r}`); if(cfg.product!=='qurl' && /(from\s+["']qrcode["']|require\(["']qrcode["']\)|QRCodeCanvas|QRCodeSVG)/.test(c)) errors.push(`Possible local QR rendering in non-qurl product: ${r}`); if(cfg.product!=='passage' && /(createSession|setSession|sessionCookie|passport\.use|NextAuth\()/i.test(c)) warnings.push(`Possible local auth/session implementation outside Passage: ${r}`); } }
