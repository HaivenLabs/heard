...
function writeConstitutionRuntime(repoRoot,cfg){
  ensureOptions(cfg);
  if(cfg.options.skills.constitution) write(path.join(repoRoot,CONSTITUTION_SKILL_PATH), constitutionSkill(cfg));
...
function upsertScripts(repoRoot){ const p=path.join(repoRoot,'package.json'); if(!exists(p)) return; const pkg=JSON.parse(read(p)); pkg.scripts=pkg.scripts||{}; pkg.scripts['haiven:sync']=pkg.scripts['haiven:sync']||'haiven sync'; pkg.scripts['haiven:check']=pkg.scripts['haiven:check']||'haiven check'; pkg.scripts['haiven:doctor']=pkg.scripts['haiven:doctor']||'haiven doctor'; write(p,JSON.stringify(pkg,null,2)); }
function ensureWorkflow(repoRoot){ const p=path.join(repoRoot,'.github/workflows/haiven-standards.yml'); if(exists(p)) return; write(p, `name: Haiven Standards\n\non:\n  pull_request:\n  push:\n    branches:\n      - main\n\njobs:\n  haiven-check:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - uses: actions/setup-node@v4\n        with:\n          node-version: 22\n      - run: corepack enable\n      - run: pnpm install\n      - run: pnpm haiven:check\n`); }
...
function check(repoRoot){
  ...
  for(const f of ['.env','.env.local','.env.development','.env.production','.env.test']) if(exists(path.join(repoRoot,f))) errors.push(`Secret-like environment file should not be committed: ${f}`);
  const pkgPath=path.join(repoRoot,'package.json');
  if(exists(pkgPath)){
    const pkg=JSON.parse(read(pkgPath));
  ...
  const files=walk(repoRoot,[],['node_modules','.git','dist','.next','build','coverage','.haiven']); for(const f of files){ const r=rel(f,repoRoot), base=path.basename(f); if(['.env','.env.local','.env.development','.env.production','.env.test'].includes(base)) errors.push(`Secret-like environment file found: ${r}`); if(/migrations?\//i.test(r) && !/^(\d{3,}|\d{4}[_-]\d{2}[_-]\d{2}|v\d+)[_-].+\.(sql|ts|js|go)$/i.test(base)) warnings.push(`Migration file may not be versioned: ${r}`); if(/\.(ts|tsx|js|jsx|go|py|cs|java)$/i.test(f)){ const c=read(f); if(/console\.log\([^)]*(password|secret|token|apiKey|api_key)/i.test(c)) errors.push(`Possible sensitive logging found: ${r}`); if(cfg.product!=='qurl' && /(from\s+["']qrcode["']|require\(["']qrcode["']\)|QRCodeCanvas|QRCodeSVG)/.test(c)) errors.push(`Possible local QR rendering in non-qurl product: ${r}`); if(cfg.product!=='passage' && /(createSession|setSession|sessionCookie|passport\.use|NextAuth\()/i.test(c)) warnings.push(`Possible local auth/session implementation outside Passage: ${r}`); } }
