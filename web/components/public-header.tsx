  ...
  return (
    <header className="relative mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-y-4 px-6 py-6">
      ...
      <nav className="order-last grid w-full grid-cols-2 items-center gap-2 sm:order-none sm:flex sm:w-auto sm:gap-2" aria-label="Primary navigation">
        ...
        <Link className="hidden rounded-full px-3 py-2 font-body text-sm text-ink/55 transition hover:text-ink lg:block" href={"/contact#walkthrough" as Route}>Walkthrough</Link>
        ...
        <Link className="rounded-full bg-clay px-3 py-2.5 text-center font-display text-xs font-semibold tracking-[0.03em] text-white shadow-[0_10px_24px_rgba(203,104,67,0.2)] transition hover:-translate-y-0.5 hover:bg-[#b95635] sm:px-5 sm:text-sm" href={"/start?source=homepage" as Route}>Start using heard</Link>
      </nav>
    </header>
  );
}
