...
export default function HomePage() {
  return (
    <main className="relative overflow-hidden bg-parchment text-ink">
      <BrandBackdrop />
      <section className="relative border-b border-ink/10" id="how-it-works">
        <PublicHeader />

        <div className="relative mx-auto max-w-7xl px-6 pb-12 pt-8 lg:py-10">
          <div className="grid items-start gap-10 lg:grid-cols-[1.06fr_0.8fr]">
            <div className="lg:pt-4">
            <h1 className="max-w-4xl font-display text-5xl leading-[0.96] tracking-[-0.065em] sm:text-6xl lg:text-[5.5rem]">The guest experience, in your hands</h1>
            <p className="mt-6 max-w-2xl font-display text-2xl leading-tight tracking-[-0.03em] text-teal sm:text-3xl">Fix the visit before it becomes the review.</p>
            <p className="mt-4 max-w-2xl font-body text-lg leading-8 text-ink/62">heard gives restaurants a guest feedback experience people actually finish, then turns every rough visit into a clear recovery task while the relationship can still be saved.</p>
            <div className="mt-8 flex flex-wrap items-center gap-4">
              <Link className="rounded-full bg-primary px-7 py-4 font-display text-sm font-semibold tracking-[0.06em] text-white shadow-[0_16px_36px_rgba(9,40,21,0.28)] transition hover:-translate-y-0.5 hover:bg-spruce" href={"/start?source=homepage" as Route}>Start using heard</Link>
            ...
            <div className="relative mx-auto w-full max-w-lg lg:mr-0">
              <div className="absolute -left-8 -top-8 h-full w-full rotate-[-3deg] rounded-[2.5rem] bg-sage/25" />
              <div className="relative rounded-[2.5rem] bg-primary p-7 text-parchment shadow-[0_40px_100px_rgba(9,40,21,0.28)] sm:p-9">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="rounded-full bg-sage/15 px-3 py-1.5 font-body text-xs font-semibold text-sage">Just received</span>
                </div>
                ...
                <div className="mt-7 rounded-[1.5rem] border border-clay/28 bg-clay/10 p-5">
                  <div className="flex items-center justify-between gap-3">
                    <p className="font-body text-xs font-bold uppercase tracking-[0.18em] text-sage">Needs follow-up</p>
          ...
          <div className="mt-12 grid gap-10 lg:grid-cols-[0.7fr_1fr] lg:gap-20">
            <div>
              <p className="font-body text-xs font-bold uppercase tracking-[0.26em] text-teal">From signal to save</p>
      ...
      <section className="relative bg-primary px-6 py-24 text-parchment">
        <div className="brand-dot-field pointer-events-none absolute inset-0 opacity-40" />
        <div className="relative mx-auto flex max-w-5xl flex-col items-center text-center">
          <p className="font-body text-xs font-bold uppercase tracking-[0.26em] text-sage">Make the next rough visit recoverable</p>
          ...
          <Link className="mt-9 rounded-full bg-teal px-8 py-4 font-display text-sm font-semibold tracking-[0.06em] text-white transition hover:-translate-y-0.5 hover:bg-sage" href={"/start?source=homepage" as Route}>Start using heard</Link>
...
function Result({ children, eyebrow, title }: { children: React.ReactNode; eyebrow: string; title: string }) {
  return <article className="rounded-[2rem] border border-ink/10 bg-surface p-7 shadow-[0_20px_50px_rgba(9,40,21,0.06)]"><p className="font-body text-xs font-bold uppercase tracking-[0.2em] text-teal">{eyebrow}</p><h3 className="mt-4 font-display text-3xl leading-tight tracking-[-0.04em]">{title}</h3><p className="mt-4 font-body text-sm leading-7 text-ink/55">{children}</p></article>;
}
