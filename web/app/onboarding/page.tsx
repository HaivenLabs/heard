...
import { HeardLogo } from "../../components/heard-logo";
...
function Onboarding({ session }: { session: Session }) {
  ...
  return (
    <main className="min-h-screen bg-parchment px-5 py-8 text-ink sm:py-12">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[0.6fr_1fr]">
          <aside>
            <p className="font-body text-xs font-bold uppercase tracking-[0.26em] text-teal">Restaurant setup</p>
            ...
            <Link className="mt-9 inline-flex font-body text-sm font-semibold text-ink/48 underline decoration-ink/20 underline-offset-4 hover:text-teal" href={"/walkthrough" as Route}>Want help? Request a walkthrough.</Link>
          </aside>

          <section className="rounded-[2rem] border border-ink/10 bg-surface p-6 shadow-soft sm:p-9">
			...
			{accountNotice ? <p aria-live="polite" className="mb-5 rounded-2xl border border-teal/20 bg-teal/10 px-4 py-3 font-body text-sm leading-6 text-ink/70">{accountNotice}</p> : null}

            {state && step === "workspace" ? (
              <form onSubmit={activateWorkspace}>
                <p className="font-body text-xs font-bold uppercase tracking-[0.2em] text-teal">Step 1 of 3</p>
                ...
                <label className="mt-4 block">
                  ...
                  <div className="flex items-center rounded-2xl border border-ink/15 bg-white px-4 focus-within:border-teal focus-within:ring-4 focus-within:ring-teal/10">
                  ...
                  <p aria-live="polite" className={`mt-2 font-body text-xs ${handleAvailability === "taken" ? "text-red-700" : handleAvailability === "available" ? "text-teal" : "text-ink/48"}`}>{handleAvailability === "checking" ? "Checking availability…" : handleAvailability === "available" ? `/${slugify(restaurantHandle)} is available.` : handleAvailability === "taken" ? `/${slugify(restaurantHandle)} is already in use. Choose another.` : "This is your restaurant’s unique public link prefix. You can change it later."}</p>
                </label>
                <button className="mt-7 h-14 w-full rounded-full bg-primary px-6 font-display text-sm font-semibold tracking-[0.06em] text-white transition hover:bg-spruce disabled:opacity-60" disabled={busy} type="submit">{busy ? "Preparing your workspace..." : "Continue to your campaign"}</button>
              </form>
            ) : null}

            {state && step === "campaign" ? (
              <form onSubmit={createFirstCampaign}>
                <p className="font-body text-xs font-bold uppercase tracking-[0.2em] text-teal">Step 2 of 3</p>
                ...
                <div className="mt-5 rounded-2xl bg-canvas p-5 font-body text-sm leading-6 text-ink/58"><strong className="text-ink">{state.tenant?.name}</strong><br />{state.location?.name} · Five-face feedback survey<br /><span className="text-xs text-ink/48">Your public link will start with /f/{state.tenant?.slug ?? "your-restaurant"}/ followed by the path you choose.</span></div>
                <button className="mt-7 h-14 w-full rounded-full bg-primary px-6 font-display text-sm font-semibold tracking-[0.06em] text-white transition hover:bg-spruce disabled:opacity-60" disabled={busy} type="submit">{busy ? "Creating your campaign..." : "Create feedback campaign"}</button>
              </form>
            ) : null}

            {state && step === "feedback_link" && state.campaign ? (
              <div>
                <p className="font-body text-xs font-bold uppercase tracking-[0.2em] text-teal">Your campaign is safe</p>
                ...
                <button className="mt-7 h-14 w-full rounded-full bg-primary px-6 font-display text-sm font-semibold text-white disabled:opacity-60" disabled={busy} onClick={() => void createCampaignLink()} type="button">{busy ? "Creating your link..." : "Finish campaign link"}</button>
              </div>
            ) : null}

            {state && step === "complete" && state.feedback_link && finishedHere ? (
              <div>
                <p className="font-body text-xs font-bold uppercase tracking-[0.2em] text-teal">Your campaign is live</p>
                <h2 className="mt-3 font-display text-3xl tracking-[-0.04em]">Ready for your first honest answer.</h2>
                <p className="mt-3 font-body text-sm leading-6 text-ink/55">Share this link now. If qurl is unavailable, the guest link still works and you can generate the QR asset later. The link prefix <code className="rounded bg-ink/5 px-1 font-mono text-teal">/f/{state.tenant?.slug ?? "your-restaurant"}/</code> is unique to your restaurant.</p>
                <a className="mt-6 block break-all rounded-2xl bg-mist px-4 py-4 font-body text-sm font-semibold text-teal" href={state.feedback_link.destination_url} rel="noreferrer" target="_blank">{state.feedback_link.destination_url}</a>
                {!state.feedback_link.qr_asset_url ? <p className="mt-3 rounded-2xl bg-sand/45 px-4 py-3 font-body text-sm text-spruce">Your link is ready. QR generation is temporarily unavailable.</p> : null}
                <div className="mt-6 flex flex-wrap gap-3">
                  <button className="rounded-full bg-primary px-6 py-3 font-body text-sm font-semibold text-white" onClick={() => void copyLink()} type="button">{copied ? "Link copied" : "Copy feedback link"}</button>
...
function Progress({ done, number, text }: { done: boolean; number: string; text: string }) {
  return <li className="flex items-center gap-3"><span className={`grid h-8 w-8 place-items-center rounded-full font-display text-xs ${done ? "bg-teal text-white" : "bg-ink/8 text-ink/50"}`}>{done ? "✓" : number}</span>{text}</li>;
}

function Field({ defaultValue, label, name, placeholder }: { defaultValue?: string; label: string; name: string; placeholder: string }) {
  return <label className="block"><span className="mb-2 block font-body text-sm font-semibold">{label}</span><input className="h-14 w-full rounded-2xl border border-ink/15 bg-white px-4 font-body outline-none transition placeholder:text-ink/30 focus:border-teal focus:ring-4 focus:ring-teal/10" defaultValue={defaultValue} name={name} placeholder={placeholder} required /></label>;
}
