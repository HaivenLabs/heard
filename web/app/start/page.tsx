      ...
      <div className="relative mx-auto grid min-h-[calc(100vh-7rem)] max-w-7xl items-center gap-14 px-6 pb-16 pt-8 lg:grid-cols-[1fr_0.82fr]">
        ...
        <section className="rounded-[2rem] border border-ink/10 bg-[#fffdf8] p-7 shadow-[0_32px_90px_rgba(23,37,29,0.16)] sm:p-10">
          ...
          {verificationRequested ? (
            ...
            <button className="flex h-14 w-full items-center justify-center gap-3 rounded-full border border-ink/15 bg-white px-6 font-display text-sm font-semibold text-ink transition hover:border-clay/50 hover:bg-[#fffdf8] disabled:cursor-not-allowed disabled:opacity-55" disabled={busy} onClick={continueWithGoogle} type="button"><span aria-hidden="true" className="font-body text-lg font-bold text-[#4285f4]">G</span>Sign up with Google</button>
            ...
            <button className="flex h-14 w-full items-center justify-center rounded-full bg-clay px-6 font-display text-sm font-semibold tracking-[0.04em] text-white transition hover:bg-[#b95635] disabled:cursor-wait disabled:opacity-65" disabled={busy} type="submit">{busy ? "Creating your account..." : "Create your account"}</button>
          </form>}
