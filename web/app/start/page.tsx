...
import { createLocalRegistration, getStoredSession, identityAuthStartUrl, isProductionAuth, safeReturnTo } from "../../lib/api";
...
export default function StartPage() {
  ...
  async function start(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (isProductionAuth()) {
        window.location.assign(identityAuthStartUrl(returnTo, { intent: "register", email }));
        return;
      }
      await createLocalRegistration(email);
      router.replace("/onboarding" as Route);
    } catch (caught) {
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#f7f1e6] px-6 py-10 text-ink">
      ...
      <div className="relative mx-auto grid min-h-[calc(100vh-5rem)] max-w-6xl items-center gap-14 lg:grid-cols-[1fr_0.82fr]">
        ...
        <section className="rounded-[2rem] border border-ink/10 bg-[#fffdf8] p-7 shadow-[0_32px_90px_rgba(23,37,29,0.16)] sm:p-10">
          ...
          <p className="mt-3 font-body text-sm leading-6 text-ink/55">We only need your work email to begin. Restaurant details come next.</p>
          <form className="mt-8 space-y-5" onSubmit={start}>
            ...
            <button className="flex h-14 w-full items-center justify-center rounded-full bg-clay px-6 font-display text-sm font-semibold tracking-[0.07em] text-white transition hover:bg-[#b95635] disabled:cursor-wait disabled:opacity-65" disabled={busy} type="submit">{busy ? "Creating account..." : "Create my heard account"}</button>
          </form>
